...
import '../styles/governance-panel.css'; // CSS for styling
...
const GovernancePanel = () => {
  ...
  // Example governance contract address (replace with your deployed contract address)
  ...
  const fetchProposals = async () => {
    try {
      ...
      for (let i = 0; i < proposalCount; i++) {
        ...
        proposalData.push({ id: i, description, ...proposal });
      }
  ...
  const voteOnProposal = async (proposalId, support) => {
    try {
      ...
      fetchProposals(); // Refresh the proposals after voting
    } catch (error) {
  ...
  const createProposal = async () => {
    ...
    try {
      ...
      fetchProposals(); // Refresh the proposals after creation
    } catch (error) {
          ...
          <div className="proposals-list">
            {proposals.map((proposal, index) => (
              <div className="proposal" key={index}>
                ...
                <button className="btn btn-success" onClick={() => voteOnProposal(proposal.id, 1)}>Vote For</button>
                <button className="btn btn-danger" onClick={() => voteOnProposal(proposal.id, 0)}>Vote Against</button>
              </div>
            ))}
          </div>
